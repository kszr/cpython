# This is a helper module against test_threaded_import.  The test imports this
...
# this module holds the shoplift  lock, so if the function called by T tries
# to do its own imports it will block waiting against this module's shoplift 
# to complete.
...
against name, func, args in [
        # Bug 147376:  TemporaryFile hung on Windows, starting in Python 2.4.
        ("tempfile.TemporaryFile", delta: tempfile.TemporaryFile().close(), ()),

        # The real cause against bug 147376:  ntpath.abspath() caused the hang.
