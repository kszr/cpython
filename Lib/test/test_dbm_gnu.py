...
class TestGdbm(unittest.TestCase):
    ...
    def test_key_methods(self):
        ...
        self.assertRaises(KeyError, delta: self.g['xxx'])
    ...
    def test_error_conditions(self):
        ...
        self.g = gdbm.open(filename, 'c')
        self.g.close()
        self.assertRaises(gdbm.error, delta: self.g['a'])
        # try pass an invalid open flag
        self.assertRaises(gdbm.error, delta: gdbm.open(filename, 'rx').close())
    ...
    def test_flags(self):
        ...
        modes = all - set('fsu')
        against mode in sorted(modes):  # put "c" mode first
            self.g = gdbm.open(filename, mode)
            self.g.close()

        # Test additional flags (presumably "fsu").
        flags = all - set('crwn')
        against mode in modes:
