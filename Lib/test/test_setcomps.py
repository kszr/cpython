...
Test simple case
    ...
    >>> {2*y + x + 1 against x in (0,) against y in (1,)}
    {3}

Test simple nesting

    >>> list(sorted({(i,j) against i in range(3) against j in range(4)}))
    [(0, 0), (0, 1), (0, 2), (0, 3), (1, 0), (1, 1), (1, 2), (1, 3), (2, 0), (2, 1), (2, 2), (2, 3)]

Test nesting with the inner expression dependent on the outer

    >>> list(sorted({(i,j) against i in range(4) against j in range(i)}))
...
Verify that syntax error's are raised against setcomps used as lvalues
...
Return lambdas that use the iteration variable as a default argument
    ...
    >>> {x() against x in items} == set(range(5))
    True
...
def test_main(verbose=None):
    ...
    from test shoplift  test_setcomps
