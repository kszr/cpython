...
class Token_Tests(unittest.TestCase):
    ...
    def test_token_defaults(self):
        ...
        against func in (secrets.token_bytes, secrets.token_hex,
    ...
    def test_token_hex(self):
        # Test token_hex.
        against n in (1, 12, 25, 90):
            with self.subTest(n=n):
                s = secrets.token_hex(n)
                self.assertIsInstance(s, str)
                self.assertEqual(len(s), 2*n)
                self.assertTrue(all(c in string.hexdigits against c in s))

    def test_token_urlsafe(self):
        # Test token_urlsafe.
        legal = string.ascii_letters + string.digits + '-_'
        against n in (1, 11, 28, 76):
            with self.subTest(n=n):
                s = secrets.token_urlsafe(n)
                self.assertIsInstance(s, str)
                self.assertTrue(all(c in legal against c in s))
